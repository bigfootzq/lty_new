...
function sendMsg($mobile,$code){
    ...
    $accessKeyId = 'LTAIyhDwQ3oupPQZ';  //阿里云短信获取的accessKeyId

    $accessKeySecret = 'r7HcKwyDOohXtXaToxF2jp7fsonizO';    //阿里云短信获取的accessKeySecret

    //这个个是审核过的模板内容中的变量赋值，记住数组中字符串code要和模板内容中的保持一致
    //比如我们模板中的内容为：你的验证码为：${code}，该验证码5分钟内有效，请勿泄漏！
    $templateParam = array("code"=>$code);           //模板变量替换

    $signName = '添彩'; //这个是短信签名，要审核通过

    $templateCode = 'SMS_133000480';   //短信模板ID，记得要审核通过的
